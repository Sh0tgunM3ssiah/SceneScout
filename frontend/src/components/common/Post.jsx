...
import Modal from '../../components/common/SignUpModal'; 
...
const Post = ({ post }) => {
	...
	const handleSharePost = async () => {
		if (navigator.share) {
			...
			const postUrl = `${window.location.origin}/posts/${post._id}`; // Using window.location.origin to get the base URL
	
			try {
				await navigator.share({
					title: `${postOwner?.fullName}'s Post`,
					text: post.text,
					url: postUrl  // This is the URL pointing to the specific post
				});
			...
			// Optionally handle the scenario where the Web Share API is not supported
	...
	return (
        <>
                    ...
                    <div className='flex justify-between mt-3'>
                        <div className='flex gap-4 items-center w-2/3 justify-between'>
                            <div
                                ...
                                onClick={!authUser ? promptLogin : () => document.getElementById("comments_modal" + post._id).showModal()}
                            >
			...
			{showLoginModal && (
                <Modal title="Login Required" onClose={closeLoginModal}>
					<p className="text-white text-center">Please log in to like or comment on this post!</p>
					<div className="flex justify-center mt-4">
						<Link
							to={{
								pathname: "/login",
								state: { from: location }  // Pass the current location in state
							}}
							className="py-2 px-4 bg-blue-500 text-white rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-700 focus:ring-opacity-50"
						>
							Go to Login Page
						</Link>
					</div>
